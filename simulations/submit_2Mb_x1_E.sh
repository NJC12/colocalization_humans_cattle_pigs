#!/bin/bash
# Launch the x1 phenotype-scaling variant of category E: 10 replicates.
#
#   E  cattle baseline (no artificial selection)  -> cattle_baseline_from_midpoint_2Mb_x1.yaml
#      (identical to cattle_baseline_from_midpoint_2Mb.yaml but
#       gwas_scaling=gtex_scaling=1, not 35)
#
# Seeds match the x35 E run exactly (E{N} -> seed 50+N -> 51..60) so each x1
...
#
# Run once from a login node:  bash submit_2Mb_x1_E.sh
...
CFG=config/cattle_baseline_from_midpoint_2Mb_x1.yaml
...
echo "== E (x1): cattle baseline, gwas_scaling=gtex_scaling=1 =="
...
echo "Submitted category E x1: ${NREP} replicates = ${NREP} controllers (none in priority/long)."
