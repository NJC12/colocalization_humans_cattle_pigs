...
if not os.path.exists(f'/n/scratch/users/n/njc12/sims/tmp/hgtex_{seed}.vcf'):
    subprocess.run(f'/n/app/bcftools/1.21-gcc-14.2.0/bin/bcftools annotate --set-id "%CHROM\\_%POS" /n/scratch/users/n/njc12/sims/tmp/hgtex_bad_ids_{seed}.vcf -o /n/scratch/users/n/njc12/sims/tmp/hgtex_{seed}.vcf', env=os.environ, shell=True)

if not os.path.exists(f'/n/scratch/users/n/njc12/sims/tmp/hgwas_{seed}.vcf'):
    subprocess.run(f'/n/app/bcftools/1.21-gcc-14.2.0/bin/bcftools annotate --set-id "%CHROM\\_%POS" /n/scratch/users/n/njc12/sims/tmp/hgwas_bad_ids_{seed}.vcf -o /n/scratch/users/n/njc12/sims/tmp/hgwas_{seed}.vcf', env=os.environ, shell=True)
