#!/bin/bash
# Launch the x10 phenotype-scaling variant of category A: 10 replicates.
#
#   A  human, directional-negative trait selection  -> human_2Mb_x10.yaml
#      (identical to human_2Mb.yaml but gwas_scaling=gtex_scaling=10, not 35)
#
# Seeds match the x35 A run exactly (A{N} -> seed 10+N -> 11..20) so each x10
...
#
# Run once from a login node:  bash submit_2Mb_x10.sh
...
CFG=config/human_2Mb_x10.yaml

echo "== A (x10): human, directional-negative, gwas_scaling=gtex_scaling=10 =="
...
echo "Submitted category A x10: ${NREP} replicates = ${NREP} controllers (none in priority/long)."
