#!/bin/bash
# Launch the x10 phenotype-scaling variant of category E: 10 replicates.
#
#   E  cattle baseline (no artificial selection)  -> cattle_baseline_from_midpoint_2Mb_x10.yaml
#      (identical to cattle_baseline_from_midpoint_2Mb.yaml but
#       gwas_scaling=gtex_scaling=10, not 35)
#
# Seeds match the x35 E run exactly (E{N} -> seed 50+N -> 51..60) so each x10
...
#
# Outputs land under a separate ..._2Mb_x10 root so x35 and x10 sit side by side
...
#
# Run once from a login node:  bash submit_2Mb_x10_E.sh
...
# Destination roots for the x10 outputs.
X10_SCRATCH=/n/scratch/users/n/njc12/snakemake/simulations_round_2_2Mb_x10
X10_PUBLISH=/n/data2/hms/dbmi/sunyaev/lab/nconnally/snakemake/simulations_round_2_2Mb_x10
cd "$REPO"

NREP=10
BASE=50                 # E{N} -> seed BASE+N -> 51..60
CFG=config/cattle_baseline_from_midpoint_2Mb_x10.yaml
...
echo "== E (x10): cattle baseline, gwas_scaling=gtex_scaling=10 =="
for N in $(seq 1 "$NREP"); do
    ...
    mkdir -p "$WD"
    JID=$(sbatch --parsable --job-name="${ID}_x10" \
...
echo "Submitted category E x10: ${NREP} replicates = ${NREP} controllers (none in priority/long)."
