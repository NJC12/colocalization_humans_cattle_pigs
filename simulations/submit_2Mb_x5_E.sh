#!/bin/bash
# Launch the x5 phenotype-scaling variant of category E: 10 replicates.
#
#   E  cattle baseline (no artificial selection)  -> cattle_baseline_from_midpoint_2Mb_x5.yaml
#      (identical to cattle_baseline_from_midpoint_2Mb.yaml but
#       gwas_scaling=gtex_scaling=5, not 35)
#
# Seeds match the x35 E run exactly (E{N} -> seed 50+N -> 51..60) so each x5
...
#
# Outputs land under a separate ..._2Mb_x5 root so x35 and x5 sit side by side
...
#
# Run once from a login node:  bash submit_2Mb_x5_E.sh
...
# Destination roots for the x5 outputs.
XOUT_SCRATCH=/n/scratch/users/n/njc12/snakemake/simulations_round_2_2Mb_x5
XOUT_PUBLISH=/n/data2/hms/dbmi/sunyaev/lab/nconnally/snakemake/simulations_round_2_2Mb_x5
cd "$REPO"

NREP=10
BASE=50                 # E{N} -> seed BASE+N -> 51..60
CFG=config/cattle_baseline_from_midpoint_2Mb_x5.yaml
...
echo "== E (x5): cattle baseline, gwas_scaling=gtex_scaling=5 =="
for N in $(seq 1 "$NREP"); do
    ...
    mkdir -p "$WD"
    JID=$(sbatch --parsable --job-name="${ID}_x5" \
...
echo "Submitted category E x5: ${NREP} replicates = ${NREP} controllers (none in priority/long)."
